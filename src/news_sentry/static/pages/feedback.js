/**
 * News Sentry — 反馈管理页面 (记录 + 规则优化双 Tab)
 */

"use strict";

import { state, api, apiPost, escapeHtml, formatDate, showSuccess, showError, scoreColor, sentimentLabelColor } from "../api.js?v=20260527c";
...
/**
 * Tab 1: 反馈记录 — 统计卡片 + 反馈列表 + publish/archive/comment 操作
 */
export async function renderFeedbackRecordsTab(container) {
  container.innerHTML = `
...
/**
 * Tab 2: 规则优化 — dry-run 预览 + 应用
 */
export async function renderRuleOptimizeTab(container) {
  container.innerHTML = `
    <div class="card">
  ...
  const btnDryRun = container.querySelector("#btnDryRun");
  const btnApply = container.querySelector("#btnApply");
  const resultEl = container.querySelector("#optimizeResult");
