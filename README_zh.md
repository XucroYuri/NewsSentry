...
**参考用例 — 意大利 Breaking News 监控：**
监控 8 个意大利新闻源（ANSA、Corriere della Sera、la Repubblica、TGCOM24、il Fatto Quotidiano、La Stampa、Il Messaggero、ANSA English），使用 91 个意大利语关键词覆盖政治、经济、犯罪、欧盟关系、中意关系、移民、能源和司法等领域。
...
```
  ...
  API 端点 ───────→ │  (8 个信源)  │
                    └──────┬───────┘
                           │
                    ┌──────▼───────┐
                    │   过滤       │ ──→ evaluated/*.md  (关键词匹配 + 分类)
                    │ (91 个关键词)│ ──→ archive/*.md     (已拒绝)
                    └──────┬───────┘
                           │
                    ┌──────▼───────┐
                    │   研判       │ ──→ evaluated/*.md  (评分 + 推荐)
                    │ (规则引擎)   │
                    └──────┬───────┘
                           │
                    ┌──────▼───────┐
                    │   输出       │ ──→ drafts/*.md      (Markdown 报告)
                    │  (Markdown)  │
                    └──────────────┘
```
...
| 阶段 | 输入 | 输出 | 说明 |
|------|------|------|------|
| **collect（采集）** | RSS/API 配置 | `raw/*.md` | 从配置的信源抓取和解析新闻 |
| **filter（过滤）** | `raw/*.md` | `evaluated/*.md` | 关键词匹配（91 关键词，词边界正则）、L0-L2 分类、去重 |
| **judge（研判）** | `evaluated/*.md` | `evaluated/*.md` | 新闻价值评分、中国相关性、推荐（publish/review/archive/discard） |
...
config/
├── targets/           # 监控目标定义
│   ├── italy.yaml     # 意大利目标（语言、信源、分类轴）
│   └── _template.yaml # 新目标模板
├── sources/italy/     # 信源配置（每个源一个 YAML）
│   ├── ansa.yaml      # 已启用
│   ├── corriere.yaml  # 已启用
│   └── ...            # 共 8 个启用，7 个禁用
├── filters/italy/     # 关键词过滤规则
│   └── default.yaml   # 91 个意大利语关键词及权重
├── classification/    # L0-L2 分类规则
│   └── rules-v1.yaml
├── profiles/          # 部署 profile
...
├── sandbox/           # 沙箱安全策略
├── runtime/           # 运行时载体配置
├── provider/          # AI provider 路由（Phase 5）
├── output/            # 输出目标配置
└── toolmanifest/      # 工具清单注册
    └── opencli-baseline.yaml  # 12 个 OpenCLI 工具
```
...
信源配置在 `config/sources/italy/*.yaml` 中。每个信源：
...
|------|------|------|------|
| ANSA | it | rss | 正常 |
| ANSA English | en | rss | 正常 |
| Corriere della Sera | it | rss | 间歇性 SSL |
| la Repubblica | it | rss | 正常 |
| TGCOM24 | it | rss | 正常 |
| il Fatto Quotidiano | it | rss | 正常 |
| La Stampa | it | rss | 正常 |
| Il Messaggero | it | rss | 正常 |

**已禁用的信源（7 个）：** AGI、Rai News、Il Sole 24 Ore、The Local Italy、Sky TG24、FAO RSS — RSS 订阅地址已永久失效。
...
│   ├── sandbox.py     # SandboxEnforcer — 沙箱策略执行（网络/文件/命令）
│   ├── file_writer.py # FileWriter — 文件事件写入（stage → 目录映射）
│   ├── memory.py      # Memory — 已知 ID、信源健康、游标、Provider 统计
│   └── run_log.py     # RunLog — 运行日志生成（阶段、错误、摘要）
├── skills/            # Pipeline 技能
│   ├── collect/
...
│   ├── filter/
│   │   ├── rules_filter.py     # 基于关键词的规则过滤器
│   │   └── classifier_rules.py # L0-L2 分类引擎
│   ├── judge/
│   │   ├── rules_judge.py      # 基于规则的评分引擎
│   │   └── judge_skill.py      # AI 研判（Phase 5 桩）
│   └── output/
│       └── markdown_writer.py  # Markdown 报告生成器
├── adapters/          # 集成桥接层
│   ├── runtime/       # Hermes Agent / OpenClaw 适配器（Phase 2 桩）
│   ├── tools/         # OpenCLI 工具适配器
│   └── providers/     # AI Provider 适配器（Phase 5 桩）
├── models/            # Pydantic v2 数据模型
│   ├── newsevent.py   # NewsEvent — 核心数据交换对象
│   ├── pipeline_context.py
│   └── manifests.py   # Tool/Skill manifest 模型
└── cli/               # Click CLI 入口
    └── __init__.py    # run、validate、skill、tool、doctor 命令
```
...
| **测试** | pytest 8.0+ | 383 个测试，89% 覆盖率 |
| **代码风格** | ruff 0.4+ | 零容忍 |
| **类型检查** | mypy 1.10+ | strict 模式，38 个源文件 |
| **CI/CD** | GitHub Actions | Python 3.11 + 3.12 矩阵 |
| **容器** | Docker (python:3.12-slim) | 数据卷挂载 |
...
| [docs/development-plan.md](docs/development-plan.md) | 七阶段开发计划 |
| [docs/adr/](docs/adr/) | 架构决策记录（ADR-0001 至 0016） |
| [docs/spec/](docs/spec/) | Phase SPEC 索引 + 组件矩阵 |
| [docs/testing/](docs/testing/) | 测试方案 + Hermes Agent 验证报告 |
...
Docker 镜像基于 `python:3.12-slim`，`/data` 挂载为数据卷，默认使用 `cloud-vps` profile。
...
| 4 — 工具/技能注册 | ✅ 完成 | OpenCLI 基线、注册表、APICollector、CLI skill/tool 命令 |
| 5 — AI Provider 路由 | ✅ 完成 | 多 Provider 路由、研判/翻译/分类、成本预算 |
| 6 — 沙箱强化 + 社媒 | ✅ 完成 | 完整沙箱策略、session profile、KOL 实验通道 |
| 7 — 多目标扩展 | ✅ 完成 | 第二目标 `china-watch-en`、硬编码扫描、目标对比文档 |
