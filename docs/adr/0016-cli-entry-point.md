...
```
news-sentry run --target <target_id> --stage <stage>
```
...
| `--config-dir` | 否 | 路径 | 覆盖默认 `./config/` 路径 |
...
```
news-sentry skill list               # 列出已注册 Skill
news-sentry tool list                # 列出已注册 ToolManifest
news-sentry run ...                  # 主运行命令（本 ADR 锁定）
news-sentry validate --config <path> # 校验 config YAML 对 schema
```
...
- Host 调用 `news-sentry run` 后可通过退出码判断状态：`0`=成功，`1`=部分失败，`2`=配置错误，`3`=沙箱阻断
