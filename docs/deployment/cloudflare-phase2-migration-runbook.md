...
120 秒、轮询间隔为 500 毫秒。该 RPC 只传可结构化克隆的数值选项，不跨 Durable Object RPC
传递 `AbortSignal`；固定启动上限仍低于 8 分钟任务预算，后续 `fetch()` 使用同一总超时信号。
若 SDK 仍返回可识别的临时启动失败，Worker 最多再尝试 5 次 `fetch()`，间隔为 5、15、30、45
秒。只有采集状态为
