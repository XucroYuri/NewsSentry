...
| Phase 86 | 旧公共门户软退场 | Done | `/public-app/` 已完成生产灰度；旧 `/#/news/*` 公开路由在客户端跳转到新 app，后台 hash 路由保留。 |
| Phase 88 | 首屏性能投影与缓存 | Done | `/api/v1/public/news` 已切到 SQLite index 优先、短 TTL projection cache 和 source 配置缓存；生产公网首屏已低于 15 秒验收线。 |
...
1. 继续观察 Phase 88 上线后 24-72 小时：health、Cloudflare Tunnel、公开新闻 API、旧公开路由跳转和后台登录入口。
2. 评估服务端默认首页替换：灰度稳定后再决定是否让 `/` 直接返回新公共 app，而不是 legacy shell 客户端跳转。
3. 制定旧公共代码清理窗口：保留后台 Vanilla JS，逐步下线旧 public route/render 模块与无用 CSS。
4. 规划公开新闻搜索 FTS：当前 `q` 仍是有界查询，后续如搜索使用频率升高，再升级为 SQLite FTS 或专用 projection。
