...
News Sentry 要建设的是一套面向驻外新闻机构、记者站和研究团队的 **可配置 Agent Skill 平台**，而不是单一国家或单一数据源的新闻爬虫。我们将开发一个框架无关的通用内核，将目标国家配置、工具适配、Skill Registry、sandbox 执行环境、AI Provider、文件事件协议和长期记忆统一到稳定架构中，使 Hermes Agent 作为第一主编排运行载体，使 OpenClaw/OpenClaw Skills/ClawHub 作为主要 Skill 生态与兼容运行载体。Codex Automations 与 Claude Desktop Cowork Scheduled Tasks 只作为备用自动化方案，用于项目维护、研究报告和人工可审阅补充任务。长期目标是让团队可以通过配置快速创建“意大利版”“法国版”“涉华议题版”等新闻监控 Skill Pack，并在可审计、可降级、可扩展的环境中持续运行。
...
Runtime Host Layer
  ...
  OpenClaw Skill runtime and ClawHub ecosystem
  Codex / Claude Cowork fallback automation
    |
    v
Runtime Adapter Layer
  runtime host adapter / heartbeat adapter / skill wrapper / context adapter
    |
    v
...
### Phase 0.5: Runtime Carrier Alignment

目标：把生产运行载体优先级和部署 profile 定稿，避免实现阶段把开发工具和生产运行框架混为一谈。

交付：

1. Hermes 主编排契约。
2. OpenClaw Skill runtime 和 ClawHub 生态接入边界。
3. `cloud-vps` 与 `local-workstation` 两套部署 profile。
4. `RuntimeHostAdapter` 最小约定。
5. Codex Automations 与 Claude Cowork Scheduled Tasks 的 fallback automation 边界。

成功标准：实现者清楚知道 Hermes 承接长期生产调度，OpenClaw 承接 Skill 生态和兼容运行，Codex/Claude Cowork 不作为 24 小时生产主监控。
...
| 核心架构 | 框架无关内核 + Hermes 主编排 + OpenClaw Skill runtime + 薄 runtime adapter | 兼容生产运行和 Skill 生态复用 |
| 运行载体 | Hermes 优先，OpenClaw 作为 Skill 生态与兼容运行载体，Codex/Claude Cowork 备用 | 24 小时监控需要稳定主调度，开发/桌面自动化不承担主链路 |
...
建议接下来补齐或持续维护以下架构规格，作为本 PRD 的工程承接：

1. [Hermes与OpenClaw运行载体规格](./Hermes与OpenClaw运行载体规格.md)
2. [ToolManifest与工具适配层规格](./ToolManifest与工具适配层规格.md)
3. [AIProvider与模型路由规格](./AIProvider与模型路由规格.md)
4. [SandboxPolicy与执行权限规格](./SandboxPolicy与执行权限规格.md)
