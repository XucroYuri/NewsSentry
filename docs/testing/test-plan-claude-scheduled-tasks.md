...
> **适用 CLI**: `python -m news_sentry.cli run --target {target_id} --stage {stage} --profile local-workstation`
...
| R-CD3a | Cowork 沙箱 Linux VM bash ≠ 主机 shell：用 `uv` 在 macOS/Windows 主机创建的 `.venv` 在 VM 内是 dangling symlink，且 VM 自带 Python 版本可能 < 3.11 | **高** |
...
#### 1.2.1 跨平台前置条件（Portability Prerequisites）

本测试方案不依赖任何主机本机路径。任意贡献者在任意设备执行只需满足：

- 已安装 Python ≥ 3.11（跨 macOS / Linux / Windows）
- 已在仓库根创建 venv，并按当前 shell 的官方方式激活该 venv：
  - POSIX / Windows 均先运行 `python -m venv .venv`
  - 激活步骤遵循本机 shell 约定，不把平台绑定解释器路径写入 automation 配置
- 已安装项目：`pip install -e ".[dev]"`
- 工作目录为仓库根（用 `git rev-parse --show-toplevel` 校验）

文档中的 `python -m news_sentry.cli ...` 一律假设当前 PATH 中的 `python` 已指向项目 venv 解释器，不写入任何绝对路径或虚拟环境内部解释器路径。

> **重要：Claude Desktop Cowork 用户**
> Cowork 的 Bash tool 运行在隔离的 Linux 沙箱 VM 中，与主机 shell 不共享文件系统语义。如需在 Cowork 内执行真实 CLI/pytest，必须在 VM 内独立准备 Python ≥ 3.11 与 venv；否则将 Scheduled Task 的 prompt 写为只做文件级证据审查（fallback 模式），把 CLI 执行委托给主机 shell。本测试方案的 §2.2/§2.3 在 fallback 模式下应跳过 live 执行步骤，改为读取既有 RunLog 与产物。

### 1.3 测试目标

1. **CLAUDE.md 正确性**: 验证 CLAUDE.md 中的入口命令、测试命令、架构指引与实际一致
2. **Tool 可用性**: 验证 Claude Desktop 的 tool set 能覆盖 News Sentry 的所有验证操作
3. **CLI 上下文可达性**: 验证 Agent 能找到正确的 Python venv，并通过 `python -m news_sentry.cli` 执行项目 CLI
...
| AC-2 | CLAUDE.md 中的 test 命令可执行并全部通过 | 281 passed |
| AC-3 | Agent 能通过 Bash tool 触发 `python -m news_sentry.cli run` | exit_code ∈ {0, 1} |
...
Bash: python --version
Bash: python -c "from news_sentry.cli import main; print('CLI OK')"
...
- [ ] `python` 可执行且能 import `news_sentry`
- [ ] 所有核心模块可 import
- [ ] pytest 全部通过
...
# Step 2: 验证 Python 环境
Bash: python -c "import news_sentry; print('News Sentry import OK')"
      → 确认当前任务环境已安装或已设置 PYTHONPATH

# Step 3: 执行 dry-run
Bash: python -m news_sentry.cli run --target italy --stage collect --profile local-workstation --dry-run
      → 预期输出 target/run_id/stage 信息

# Step 4: 执行实际采集
Bash: python -m news_sentry.cli run --target italy --stage collect --profile local-workstation
      → 预期 30-60 秒完成
...
Bash: cat $(ls -t data/italy/logs/*.json | head -1) | python -c "
import sys, json
...
#### 2.2.4 Act
若 Python 环境不可导入项目：
- 运行 `python -m pip install -e ".[dev,proxy]"`
- 或在任务环境设置 `PYTHONPATH=${project_root}/src`
...
```
# Step 1: 执行全链路
Bash: python -m news_sentry.cli run --target italy --stage all --profile local-workstation
      → 预期 2-3 分钟完成
...
# Step 4: 运行质量门
Bash: python -m ruff check 2>&1 | tail -3
Bash: python -m mypy src/news_sentry/ 2>&1 | tail -3
Bash: python -m pytest tests/ -q 2>&1 | tail -5
```
...
Bash: python -m news_sentry.cli run --target italy --stage collect --profile local-workstation  # 实际 RSS 采集
Bash: ls ./data/logs/ 2>/dev/null  # 项目内日志目录（跨平台）
...
```json
{
  "permission_matrix": [
    ...
    {"tool": "Bash", "target": "python -m news_sentry.cli", "required": "no"},
    {"tool": "Bash", "target": "python -m news_sentry.cli run", "required": "yes", "reason": "网络出站"},
  ...
  "blockers": [],
  "recommendations": [
    "将 python -m news_sentry.cli 的限定参数设为 Bash auto-allow",
    "将 data/ 设为 Write auto-allow 目录",
    "python -m news_sentry.cli 命令设为 Bash auto-allow（仅限特定参数）"
  ]
}
```
...
| Python 环境可达性 | Bash + import 检查 | `python -c "import news_sentry"` 成功 |
...
| R-CD3 Python 环境未激活 | 使用 `python -m news_sentry.cli` 并在任务环境安装项目或设置 PYTHONPATH | `python -c "import news_sentry"` |
