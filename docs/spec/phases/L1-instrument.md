...
| # | 检查 | 结果 |
|---|------|------|
| 1 | **A**：加之前，连续 3 次 preview 部署 | ✅ 全绿（run `35449872399`、`35450729061` 等） |
| 2 | **B**：加之后，同一条流水线 | ❌ 部署失败，错误**指名该 namespace id** |
| 3 | **A′**：`git revert` 之后 | ✅ **恢复全绿**（run `35453316737`：Deploy + Verify 均 success） |
| 4 | **唯一变量** | ✅ `main` 与 `preview` 的差异只有那 11 行 |
| 5 | **失败是否伤到线上** | ✅ **未伤**：preview worker 仍为上一次成功版本（`a368edaf` / `d019b00`），health `ok`，公共读 200 / `snapshot: hit` |

**结论**：这是一次 **A-B-A 反向设计**（加变量→失败→移除变量→恢复），
是单变量实验中最强的证据形式 —— 它排除了"恰好赶上 Cloudflare 抖动"这类混淆解释
（抖动不会恰好随 revert 而消失，且错误信息直接指名该 id）。
实验全程单变量、隔离轨道、可回滚、**失败 fail-closed**：
Cloudflare 在上传阶段拒绝，在线 worker 不受影响。
...
> **状态**：已完成并提升到 `main`（2026-09-19）。**阻塞 B3 已解除（配置级）。**

### 10.1 改动

删除顶层 `[[kv_namespaces]]` 块（4 行），替换为 11 行说明性注释（记录移除原因、为何选择移除而非补建、
以及将来若需 KV 的约束）。**两轨形态恢复一致**：均无任何 KV 绑定。

### 10.2 验收证据

| # | 检查 | 结果 |
|---|------|------|
| 1 | 解析后配置无任何 KV 绑定（递归遍历顶层与各 env） | ✅ 零命中；顶层键列表中 `kv_namespaces` 已消失 |
| 2 | 渲染仍成功，且 diff 仍**恰好 1 行** | ✅ |
| 3 | `ruff` | ✅ 0 |
| 4 | 守卫测试 | ✅ `17 passed` |
| 5 | Worker 测试 | ✅ `218/218 passed` |
| 6 | **回归防线有效性**：重新注入占位 KV | ✅ 测试**立即失败** |
| 7 | **preview 流水线** | ✅ **全绿**（run `35455324360`：CI Gate / Deploy preview Worker / Verify preview 均 success） |

**第 7 项中值得单列的一点**：该 run 的 `CI Gate` 包含对**生产配置**的 `wrangler deploy --env="" --dry-run`，
且通过。即生产配置现在至少通过了静态校验。

### 10.3 提升

`preview` → `main`（fast-forward，单提交），2 文件 `+51 / −7`。

### 10.4 精确的结论边界（避免过度声称）

| 声称 | 是否成立 |
|------|---------|
| 10042 这一**特定阻断已被结构性消除** | ✅ 成立 —— 该绑定已不存在，故不可能再因它被拒 |
| 生产**现在可以部署** | ⚠️ **未验证** —— dry-run 不校验 namespace，真实的 10042 只在真部署时触发；移除后该失败模式已无来源，但"生产部署端到端成功"自 2026-08-03 起从未发生过 |
| 已执行生产部署 | ❌ **未执行** —— 本步骤只解除配置级阻断。实际部署 45 天的提交仍是独立决策 |

**因此 B3 的登记状态是"配置级已解除，端到端未验证"**，不得表述为"生产已恢复可部署并验证"。

### 10.5 一个过程教训

执行期间我用 `git checkout frontend/cloudflare/wrangler.toml` 撤销一次临时注入的回归测试，
**该命令同时还原了我尚未提交的删除**（`git checkout <file>` 恢复的是 HEAD 版本）。
发现后重新施加删除并**先提交、再跑破坏性命令**。
