...
## §3.4 已知债务：`tools/dev_progress.py` 有两个独立缺陷

该工具名为"报告本地/远端同步与路线图阶段进度"，但**自 2026-05-11 起从未可用**。
两个缺陷互相独立，**均非本轮引入**：

| # | 缺陷 | 证据 | 说明 |
|---|------|------|------|
| 1 | `SPEC_INDEX` 指向不存在的路径 | `tools/dev_progress.py:15` 自 `5e915b3`（2026-05-11）起写 `docs/spec/README.md` | 路径当时不存在；本轮创建后**格式仍不匹配**（工具期望 `\| Phase N \| ... \| status \|`，新体系用 `\| L0 \| 名称 \| \`状态\` \|`） |
| 2 | 解析 pytest 输出时崩溃 | `collect_test_metrics()` 对 `5448/5605 tests collected` 执行 `int(line.split()[0])` → `ValueError` | pytest 加入 deselect 后输出变为 `N/M` 形式，工具随即完全不可用 |

**本轮改动对缺陷 1 是行为中性的**：文件缺失时函数提前返回 `[]`；
文件存在但解析 0 行同样得到 `[]`。**因此本体系既未修复也未加重该问题**，仅如实登记。

**若后续修复**：应把 `SPEC_INDEX` 改为 `docs/spec/03-phase-plan.md`，
并按新的状态矩阵格式解析（该矩阵本身是 `tools/spec_guard.py --render` 的生成物，
因此修复后该工具将自动跟随唯一状态源）。
