...
| 存储 | SQLite (aiosqlite) + 文件系统 (YAML/Markdown) |
| AI Provider | Gemini → DeepSeek → Groq → Cloudflare (链式降级) |
| 部署 | Docker Compose / systemd (BWH VPS) / Cloudflare CDN |
...
```mermaid
flowchart TB
    USER["用户"] --> CF["Cloudflare CDN"]
    CF --> NGINX["Nginx (BWH VPS)"]
    NGINX --> NS["news-sentry<br/>uvicorn :18080"]
    NS --> RSSB["rss-bridge<br/>:13080 (内网)"]
    NS -.-> API["AI APIs<br/>(Gemini/DeepSeek/Groq)"]
```
...
| 模式 | 启动方式 | 适用 |
|------|---------|------|
| Docker | `docker compose up -d` | 开发 / 单机生产 |
| Systemd | GitHub Actions → SSH → systemd | VPS 生产 |

### CI/CD

```
Push to main → CI Gate (lint + mypy + pytest + frontend build)
             → Deploy (SSH to VPS → git pull → pip install → systemd restart)
```
