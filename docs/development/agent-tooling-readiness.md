...
> 日期: 2026-05-09
> 范围: Codex、opencode、Claude Code、Cursor 等开发工具的项目级协作准备

## 1. 目标

本项目后续会同时使用 Codex、opencode、Claude Code、Cursor 等 Agent 化开发工具。准备工作的核心不是为每个工具维护一套不同规则，而是建立一个共享工程上下文，让不同工具进入仓库后都能理解同一组架构边界、开发阶段和安全约束。
...
5. 开发工具不改变项目路线：第一阶段仍以 Contract Stabilization 和 Kernel MVP 为主，不提前把 OpenCLI、社媒登录态、复杂 Provider 或数据库队列拉进 v1。
...
4. 后续 Phase 1 实现任务可以直接引用这些入口文件，而不需要重新解释项目边界。
