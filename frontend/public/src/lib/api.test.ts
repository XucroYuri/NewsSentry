...
describe("public news API client", () => {
  ...
  it("loads the public bootstrap payload for first paint", async () => {
    ...
    const result = await getPublicBootstrap({ featured: true, pageSize: 20 }, { fetcher })

    expect(fetcher).toHaveBeenCalledWith("/api/v1/public/bootstrap?featured=true&page_size=20", {
      signal: undefined,
    })
